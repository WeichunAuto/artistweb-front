      ...
      (async () => {
        const updatedMainData = await Promise.all(
          tempTopicsData.map(async (aTopic) => {
            try {
              const response = await axiosInstance.get(`/getATopic/${aTopic.id}/image`,
                {
                  responseType: "blob",
                  headers: {
                    'Authorization': 'Bearer ' + jwtToken
        ...
        <div className='w-full h-auto relative pt-10 pb-6 bg-pink-400'>
          ...
          {/* perspectives */}
          {
            topics.map((topic, index) => {
              let reverseStyle = index % 2 === 0 ? 'lg:flex-row-reverse lg:pl-16' : 'lg:flex-row'
              return (
                <div key={index} className={`py-5 sm:py-5 lg:py-10 w-11/12 sm:w-11/12 lg:w-[98%] h-auto lg:h-[630px] mx-auto mt-2 bg-white rounded-lg drop-shadow-lg flex flex-col sm:flex-col ${reverseStyle} justify-center justify-items-center`}>
